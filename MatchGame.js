...
class App extends Component {
  ...
  startNewGame = () => {
    this.setState({
      score: 0,
      mainImageUrl: imagesList[0].imageUrl,
      timeLeft: 60,
      gameOver: false,
      activeTab: tabsList[0].tabId,
    })
  ...
  handleTabClick = tabId => {
    const categoryImages = imagesList.filter(img => img.category === tabId)
    const randomIndex = Math.floor(Math.random() * categoryImages.length)

    this.setState({
      activeTab: tabId,
      mainImageUrl: categoryImages[randomIndex].imageUrl,
    })
  }

  getFilteredImages = () => {
    const {activeTab} = this.state
    return imagesList.filter(eachItem => eachItem.category === activeTab)
  }
  ...
  onThumbnailClick = clickedId => {
    const {mainImageUrl} = this.state
    const currentMainImage = imagesList.find(
      image => image.imageUrl === mainImageUrl,
    )
    if (currentMainImage && clickedId === currentMainImage.id) {
      const randomImage = this.generateRandomImage()
      this.setState(prevState => ({
        score: prevState.score + 1,
        mainImageUrl: randomImage.imageUrl,
      }))
    } else {
      clearInterval(this.timer)
      this.setState({
  ...
  renderGameScreen = () => {
    const {activeTab, mainImageUrl} = this.state
    const filteredImages = this.getFilteredImages()

    return (
      <div className="bottom-container">
        <img src={mainImageUrl} className="random-image" alt="match" />
        <ul className="tabs-button-container">
          {tabsList.map(eachTabItem => (
            <li
              key={eachTabItem.tabId}
              onClick={() => this.handleTabClick(eachTabItem.tabId)}
            >
              <button
                type="button"
                className={
                  activeTab === eachTabItem.tabId
                    ? 'active-tab-button'
                    : 'each-tab-button'
                }
              >
        ...
        <ul className="thumbnails-container">
          {filteredImages.map(eachItem => (
            <li
              className="list-item"
              key={eachItem.id}
              onClick={() => this.onThumbnailClick(eachItem.id)}
            >
              <button type="button">
                <img
  ...
  render() {
    const {score, timeLeft, gameOver} = this.state
      ...
      <div className="bg-container">
        <ul className="navbar-container">
          ...
          <div className="score-container">
            ...
            <li>
              <p className="score-count">{timeLeft} sec</p>
            </li>
          </div>
        </ul>
        {gameOver ? this.renderGameOver() : this.renderGameScreen()}
      </div>
    )
  }
}
